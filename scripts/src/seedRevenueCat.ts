// Seeds RevenueCat for Wistoria: a single "pro" entitlement unlocked by either
// a monthly or annual subscription, grouped under the "default" offering.
// Run once: pnpm --filter @workspace/scripts exec tsx src/seedRevenueCat.ts
// Authenticated via the Replit RevenueCat connector (see revenueCatClient.ts).
...
const ENTITLEMENT_DISPLAY_NAME = "Wistoria Pro";

const OFFERING_IDENTIFIER = "default";
const OFFERING_DISPLAY_NAME = "Wistoria Pro";

type PlanConfig = {
  // Shared store identifier for test store + app store.
  storeIdentifier: string;
  // Play store requires {subscriptionId}:{basePlanId}.
  playStoreIdentifier: string;
  displayName: string;
  userFacingTitle: string;
  // P1W P1M P2M P3M P6M P1Y
...
const PLANS: PlanConfig[] = [
  {
    storeIdentifier: "wistoria_pro_monthly",
    playStoreIdentifier: "wistoria_pro_monthly:monthly",
    displayName: "Wistoria Pro Monthly",
    userFacingTitle: "Wistoria Pro (Monthly)",
    duration: "P1M",
  ...
  {
    storeIdentifier: "wistoria_pro_annual",
    playStoreIdentifier: "wistoria_pro_annual:annual",
    displayName: "Wistoria Pro Annual",
    userFacingTitle: "Wistoria Pro (Annual)",
    duration: "P1Y",
    packageIdentifier: "$rc_annual",
    packageDisplayName: "Annual",
    prices: [{ amount_micros: 59990000, currency: "USD" }],
  },
];
...
async function seedRevenueCat() {
  ...
  const addTestStorePrices = async (testStoreProduct: Product, plan: PlanConfig) => {
    ...
    if (priceError) {
      if (
        ...
        console.log("Test store prices already exist for", plan.storeIdentifier);
      } else {
        throw new Error("Failed to add test store prices for " + plan.storeIdentifier);
      }
    } else {
      console.log("Added test store prices for", plan.storeIdentifier);
    }
  };

  // Per-plan products keyed by package identifier.
  const planProducts: Record<
    string,
    { plan: PlanConfig; test: Product; appStore: Product; playStore: Product }
  > = {};

  for (const plan of PLANS) {
    const testStoreProduct = await ensureProductForApp(app, "Test Store", plan, plan.storeIdentifier, true);
    const appStoreProduct = await ensureProductForApp(appStoreApp, "App Store", plan, plan.storeIdentifier, false);
    const playStoreProduct = await ensureProductForApp(
      playStoreApp,
      "Play Store",
      plan,
      plan.playStoreIdentifier,
      false,
    );
    await addTestStorePrices(testStoreProduct, plan);
    planProducts[plan.packageIdentifier] = {
      plan,
      test: testStoreProduct,
      appStore: appStoreProduct,
      playStore: playStoreProduct,
    };
  }

  const allProductIds = Object.values(planProducts).flatMap((p) => [p.test.id, p.appStore.id, p.playStore.id]);
  ...
  for (const { plan, test, appStore, playStore } of Object.values(planProducts)) {
    let pkg: Package | undefined = existingPackages.items?.find((p) => p.lookup_key === plan.packageIdentifier);

    if (pkg) {
      console.log("Package already exists:", plan.packageIdentifier, pkg.id);
    } else {
    ...
    if (attachPackageError) {
      if (
        ...
        console.log("Skipping package attach: package already has incompatible product", plan.packageIdentifier);
      } else {
